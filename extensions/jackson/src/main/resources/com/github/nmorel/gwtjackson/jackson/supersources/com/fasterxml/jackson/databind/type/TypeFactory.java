...
public final class TypeFactory
    ...
    public JavaType constructSpecializedType(JavaType baseType, Class<?> subclass) {
        ...
        do { // bogus loop to be able to break
            ...
            if (!ClassUtils.isAssignableFrom(rawBase, subclass)) {
        ...
        if (baseType instanceof SimpleType) {
            ...
            if (subclass.isArray()
                || ClassUtils.isAssignableFrom(Map.class,subclass)
                || ClassUtils.isAssignableFrom(Collection.class,subclass)) {
                // need to assert type compatibility...
                if (!ClassUtils.isAssignableFrom(baseType.getRawClass(),subclass)) {
    ...
    public JavaType constructGeneralizedType(JavaType baseType, Class<?> superClass) {
        ...
        if (superType == null) {
            ...
            if (!ClassUtils.isAssignableFrom(superClass, rawBase)) {
                throw new IllegalArgumentException(StringUtil.format(
    ...
    public JavaType moreSpecificType(JavaType type1, JavaType type2) {
        ...
        if (ClassUtils.isAssignableFrom(raw1, raw2)) {
            return type2;
        }
        return type1;
    }
    ...
    private JavaType _referenceType(Class<?> rawClass, TypeBindings bindings,
        ...
        if (typeParams.isEmpty()) {
            ...
            throw new IllegalArgumentException("Strange Reference type " + rawClass.getName()
                    + ": can not determine type parameters");
        }
    ...
    protected JavaType _findWellKnownSimple(Class<?> clz) {
        if (clz.isPrimitive()) {
            ...
            if (clz == CLS_OBJECT)
                return CORE_TYPE_OBJECT; // since 2.7
        }
        return null;
    }
